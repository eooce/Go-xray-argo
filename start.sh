...
run() {
  ...
  if [ -e ${FILE_PATH}/web ]; then
    ...
    nohup ${FILE_PATH}/web -c ${FILE_PATH}/config.json >/dev/null 2>&1 &
	  sleep 2
    pgrep -x "web" > /dev/null && echo -e "\e[1;32mweb is running\e[0m" || { echo "web is not running, restarting..."; pkill -x "web" && nohup ${FILE_PATH}/web -c ${FILE_PATH}/config.json >/dev/null 2>&1 & sleep 2; eecho -e "\e[1;32mweb restarted\e[0m"; }
  fi
